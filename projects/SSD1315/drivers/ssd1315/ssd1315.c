...
/**
 * @brief Set the display brightness (contrast).
 * 
 * @param pObj Pointer to the SSD1315 object.
 * @param Brightness Brightness level (0x00 to 0xFF).
 * @return int32_t Returns SSD1315_OK on success, SSD1315_ERROR on failure.
 */
int32_t SSD1315_SetBrightness(SSD1315_Object_t *pObj, uint32_t Brightness) {
    uint8_t cmd[2];

    // Helligkeitswert validieren
    if (Brightness > 0xFF) {
        return SSD1315_ERROR; // Ungültiger Helligkeitswert
    }

    // SSD1315-Befehl für Helligkeit/Kontrast
    cmd[0] = 0x81;       // Command zur Einstellung des Kontrasts
    cmd[1] = (uint8_t)Brightness;

    // Befehl senden
    if (pObj->IO.WriteReg(0x00, cmd, sizeof(cmd)) != 0) {
        return SSD1315_ERROR; // Fehler beim Schreiben
    }
...
int32_t SSD1315_Refresh(SSD1315_Object_t *pObj)
{
    ...
    for (uint8_t page = 0; page < 8; page++) {
        ...
        cmd[0] = 0xB0 | page; // Page address
        ret += ssd1315_write_reg(&pObj->Ctx, 0x00, &cmd[0], 1);

        // Set column start address
        cmd[1] = 0x00; // Lower column start address
        cmd[2] = 0x10; // Higher column start address
        ret += ssd1315_write_reg(&pObj->Ctx, 0x00, &cmd[1], 2);

        // Write framebuffer data for the current page
        ret += ssd1315_write_reg(&pObj->Ctx, 0x40, &PhysFrameBuffer[page * 128], 128);
    }

    return ret == SSD1315_OK ? SSD1315_OK : SSD1315_ERROR;
}
/**
